...
function auth() {
  ...
  function initSession(idToken, nodeCallback) {
      ...
      var createSession = function(err, googleSub) {
        if (err) {
          ...
          "act": "create",
          "type": 'accountSession',
          "fields": {'token':idToken, 'accountId': googleSub}
          }, function (error, data) {
            if (error) {
                 nodeCallback(error);
            } else {
                 nodeCallback(null, googleSub);
            }
          });
        }
      }
  
      request(options, handleGoogleResponse(idToken, createSession)).end();
